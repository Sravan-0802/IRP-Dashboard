...
function StatCard({
  ...
  const color = tone === "gold" ? "#ffc83d" : tone === "blue" ? "#5b8cff" : "#46d39b";
  const grad =
    tone === "gold"
      ? "linear-gradient(90deg,#ffd870,#ffa500)"
      : tone === "blue"
        ? "linear-gradient(90deg,#5b8cff,#8a6eff)"
        : "linear-gradient(90deg,#1d9e75,#46d39b)";
  return (
    <IrpCard className="p-5">
      <div className="flex items-center justify-between">
        <p className="text-xs font-bold uppercase tracking-wider text-[#a99fce]">{label}</p>
        <span className="font-display text-sm font-extrabold" style={{ color }}>{pct}%</span>
      </div>
      <p className="mt-2 font-display text-2xl font-black text-[#e8e6ff]">
        {done}
        <span className="text-base font-medium text-[#6a6092]">/{total}</span>
      </p>
...
export function MyLearning({ subjects }: { subjects: SubjectRow[] }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-extrabold text-[#e8e6ff] sm:text-3xl">My Learning</h1>
        <p className="mt-1 text-sm text-[#7a6eaa]">Track your courses, subjects and practice progress.</p>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Overall completion" done={overallDone} total={overallTotal} tone="gold" />
        <StatCard label="MCQs done" done={mcqDone} total={mcqTotal} tone="blue" />
        <StatCard label="Problems solved" done={codeDone} total={codeTotal} tone="green" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {TRACKS.map((track) => {
          ...
          const tone = pct === 100 ? "green" : pct > 0 ? "purple" : "purple";
            ...
            <IrpCard key={track.name} className="p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-display text-base font-extrabold text-[#e8e6ff]">
                    {track.emoji} {track.name}
                  </p>
                  <p className="text-xs text-[#7a6eaa]">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {track.courses.map((c) => (
                      <span
                        key={c}
                        className="rounded-md border border-white/5 bg-white/[0.03] px-2 py-1 text-[10px] font-medium text-[#a99fce]"
                      >
                        {c}
                      </span>
                    ))}
              ...
              <button
                ...
                className="mt-4 flex w-full items-center justify-center gap-1.5 rounded-lg border border-white/5 bg-white/[0.02] py-2 text-xs font-bold text-[#7a6eaa] hover:text-[#cfc7ee]"
              >
                Subject-wise stats
              ...
              {isOpen && (
                <div className="mt-3 space-y-3 rounded-xl border border-white/5 bg-black/20 p-3">
                  {tSubjects.map((s) => (
                    <div key={s.subject}>
                      <p className="mb-1.5 text-xs font-bold text-[#e8e6ff]">{s.subject}</p>
