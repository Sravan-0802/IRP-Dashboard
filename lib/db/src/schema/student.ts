...
export const academyUserAssessmentDetailsTable = pgTable(
  "academy_user_assessment_details",
  {
    ...
    level: text("level"),
    cycle: text("cycle"),
    assessmentStartDatetime: timestamp("assessment_start_datetime", { withTimezone: true }),
    assessmentEndDatetime: timestamp("assessment_end_datetime", { withTimezone: true }),
    /** BQ column is misspelled `user_assesment_start_datetime` (one 's'). */
    userAssessmentStartDatetime: timestamp("user_assessment_start_datetime", {
      withTimezone: true,
    }),
    ...
    feSectionMaxScore: real("fe_section_max_score"),
    feUserSectionScore: real("fe_user_section_score"),
